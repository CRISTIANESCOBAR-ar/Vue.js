...
// Returns all TENSORAPID_PAR records for mapping USTER_TESTNR to TENSORAPID TESTNR
app.get('/api/tensorapid/par', async (req, res) => {
  ...
  try {
    ...
    const sql = `SELECT TESTNR, USTER_TESTNR FROM ${SCHEMA_PREFIX}TENSORAPID_PAR ORDER BY TESTNR`
    const result = await conn.execute(sql, {}, { outFormat: oracledb.OUT_FORMAT_OBJECT })
    const rows = result.rows || []
    res.json({ rows })
  } catch (err) {

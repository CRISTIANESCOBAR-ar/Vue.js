            ...
            <div class="flex flex-col gap-2 mb-3 flex-shrink-0">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button @click="exportToExcel" title="Exportar a Excel (XLSX)"
            ...
            <div v-else class="flex-1 min-h-0 flex flex-col">
                ...
                <div v-else class="flex-1 min-h-0 flex flex-col">
                    ...
                    <div
                        ...
                        <table class="min-w-full w-full table-auto divide-y divide-slate-200 text-xs">
                            ...
                            <thead class="bg-gradient-to-r from-slate-50 to-slate-100 sticky top-0 z-20">
                                <tr>
                                    <th
                                        class="px-2 py-[0.3rem] text-center font-semibold text-slate-700 border-b border-slate-200">
                                        Ensayo
                                    </th>
                                    <th
                                        class="px-2 py-[0.3rem] text-center font-semibold text-slate-700 border-b border-slate-200">
                                    ...
                                    <th
                                        class="px-3 py-3 text-center font-semibold text-slate-700 border-b border-slate-200">
                                        Delg -30%
                                    </th>
                                    <th
                                        class="px-3 py-3 text-center font-semibold text-slate-700 border-b border-slate-200">
                                    ...
                                    <th
                                        class="px-2 py-2 text-center font-semibold text-slate-700 border-b border-slate-200">
                                        Fuerza B</th>
                                    <th
                                        class="px-2 py-2 text-center font-semibold text-slate-700 border-b border-slate-200">
                                        Elong. %</th>
                    ...
                    <div class="flex items-center justify-between mt-3 px-1 flex-shrink-0">
                        <div class="text-sm text-slate-600">Mostrando {{ startDisplay }}–{{ endDisplay }} de {{
                            filteredRows.length
                            }}</div>
                        <div class="flex items-center gap-2">
                            ...
                            <select v-model.number="pageSize"
                                ...
                                <option v-for="s in [10, 25, 50, 100, 0]" :key="s" :value="s">{{ s === 0 ? 'Todos' : s
                                    }}</option>
                            </select>

                            <!-- first / prev -->
                            <button @click="page = 1" :disabled="page <= 1"
                                class="px-2 py-1 bg-slate-100 disabled:opacity-50 rounded-md text-sm" title="Primera">«
                                Primera</button>
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <input id="gotoPage" type="number" min="1" :max="totalPages"
                                    v-model.number.lazy="gotoPage" @keydown.enter.prevent="goToPage()"
                                ...
                                <button @click="goToPage()"
                                    class="px-2 py-1 bg-slate-100 rounded-md text-sm text-slate-700">Ir</button>
                            </div>

                            <!-- page indicator and next/last -->
                            <span class="text-sm text-slate-600">Página {{ page }} / {{ totalPages }}</span>
                            <button @click="page = Math.min(totalPages, page + 1)" :disabled="page >= totalPages"
                                class="px-2 py-1 bg-slate-100 disabled:opacity-50 rounded-md text-sm"
                                title="Siguiente">Siguiente
                                ›</button>
        ...
        <div v-if="modalVisible" class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog"
            ...
            <div class="bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[95vh] flex flex-col p-3 z-50 relative"
                ...
                <header class="flex items-start sm:items-center justify-between mb-2 pb-1 gap-3">
                    <div id="modalTitle" class="flex flex-col sm:flex-row sm:items-center gap-2 mx-8">
                        ...
                        <div class="text-slate-600 text-sm">Ensayo Uster <span
                                class="text-slate-900 text-lg font-semibold ml-1">{{
                                modalMeta.u }}</span> y TensoRapid <span
                                class="text-slate-900 text-lg font-semibold ml-1">{{
                ...
                <section class="flex-1 relative">
                    ...
                    <div>
                        <div v-if="mergedRows.length === 0" class="text-sm text-slate-600 py-8 text-center">No hay datos
                            para este
                            ensayo.</div>
                        <div v-else
                            class="rounded-xl border border-slate-200 overflow-auto modal-scroll max-h-[calc(95vh-4rem)]">
                     ...
                     the header sticks correctly inside the scrolling area. -->
                                ...
                                <tbody>
                                    <tr v-for="(row, idx) in mergedRows" :key="idx"
                                        ...
                                        <td class="px-3 py-1 text-center border-b border-slate-100 text-slate-700">
                                            <template v-if="modalLoading">
                                                <div class="mx-auto w-10 h-4 bg-slate-200/70 rounded animate-pulse">
                                                </div>
                                            </template>
                                        ...
                                        <td class="px-3 py-1 text-center border-b border-slate-100 text-slate-700">
                                            <template v-if="modalLoading">
                                                <div class="mx-auto w-32 h-4 bg-slate-200/70 rounded animate-pulse">
                                                </div>
                                            </template>
                                        ...
                                        <td class="px-3 py-1 text-center border-b border-slate-100 text-slate-700">
                                            <template v-if="modalLoading">
                                                <div class="mx-auto w-14 h-4 bg-slate-200/70 rounded animate-pulse">
                                                </div>
                                            </template>
                                        ...
                                        <td class="px-3 py-1 text-center border-b border-slate-100 text-slate-700">
                                            <template v-if="modalLoading">
                                                <div class="mx-auto w-16 h-4 bg-slate-200/70 rounded animate-pulse">
                                                </div>
                                            </template>
                                    ...
                                    <tr
                                        ...
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS30_KM.avg) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS40_KM.avg) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS50_KM.avg) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                    ...
                                    <tr class="bg-blue-50/50 font-medium">
                                        ...
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS30_KM.cv) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS40_KM.cv) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS50_KM.cv) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                    ...
                                    <tr class="bg-indigo-50/50 font-medium">
                                        ...
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS30_KM.sd) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS40_KM.sd) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS50_KM.sd) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                    ...
                                    <tr class="bg-blue-50/50 font-medium">
                                        ...
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS30_KM.q95) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS40_KM.q95) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS50_KM.q95) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                    ...
                                    <tr class="bg-indigo-50/50 font-medium">
                                        ...
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS30_KM.max) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS40_KM.max) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS50_KM.max) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                    ...
                                    <tr class="bg-blue-50/50 font-medium">
                                        ...
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS30_KM.min) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS40_KM.min) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
                                            fmtStat(combinedStats.DELG_MINUS50_KM.min) }}
                                        </td>
                                        <td class="px-3 py-1 text-center text-slate-700">{{
...
const pagedRows = computed(() => {
    ...
    async function copyModalAsImage() {
        try {
            ...
            try {
                ...
                try {
                    // Try to copy to clipboard (use globalThis to avoid linter no-undef)
    ...
    function exportToExcel() {
        try {
            ...
            const blob = new globalThis.Blob([wbout], { type: 'application/octet-stream' })
